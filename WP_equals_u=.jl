...
file_D = matopen(raw"C:\Users\minboli\Desktop\matlab data\Xincode\Xincode\D.mat")
D = read(file_D,"D")
...
#m refers to model, W*P<=U,P0=D*P+b
    # upper_reshape = vcat(upper_vars...)
    # #joint elements in upper_vars together
    # lower_reshape = vcat(lower_vars...)
    ...
    @constraint(m, p0_u .== D * upper_vars + b) 
    @constraint(m, p0_l .== D * lower_vars + b)
